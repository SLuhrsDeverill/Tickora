...
import { Ticket, Clock, CheckCircle, TrendingUp, Monitor, Plus, AlertCircle } from 'lucide-react';
      ...
      {!isIT && (
        <div className="space-y-4">
          <div className="bg-gradient-to-r from-blue-600 to-blue-700 rounded-xl p-6 text-white">
            <h2 className="text-xl font-semibold mb-1">Hola, {user?.firstName} 👋</h2>
            <p className="text-blue-100 text-sm mb-4">¿Tenés algún problema con tu equipo o sistema? Reportalo acá y el equipo IT te va a ayudar.</p>
            <Link
              to="/tickets/new"
              className="inline-flex items-center gap-2 bg-white text-blue-600 font-semibold px-5 py-2.5 rounded-lg text-sm hover:bg-blue-50 transition-colors"
            >
          ...
          <div className="grid grid-cols-3 gap-4">
            {[
              { label: 'Abiertos', count: recentTickets.filter(t => t.status === 'OPEN').length, color: 'blue', icon: <AlertCircle size={18} /> },
              { label: 'En progreso', count: recentTickets.filter(t => t.status === 'IN_PROGRESS').length, color: 'amber', icon: <Clock size={18} /> },
              { label: 'Resueltos', count: recentTickets.filter(t => t.status === 'RESOLVED' || t.status === 'CLOSED').length, color: 'green', icon: <CheckCircle size={18} /> },
            ].map(({ label, count, color, icon }) => (
              <div key={label} className="bg-white rounded-xl p-4 shadow-sm border border-gray-100 flex items-center gap-3">
                <div className={`w-9 h-9 rounded-lg flex items-center justify-center text-${color}-600 bg-${color}-50`}>{icon}</div>
                <div>
